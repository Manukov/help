...
### AOP
AOP () — это [парадигма программирования](#paradigm-programming) направленная на решение проблемы [запутанности кода](#code-tangling) и проблемы [code scattering](#code-scattering). Идея парадигмы АОП заключается в разделении кода на 2 группы:
* основную функциональность - бизнес-логика;
* [служебную (сквозную функциональность)](#cross-cutting-concern) - эта функциональность записывается в Aspect-классы и код сквозных функций вызывается без изменения исходного кода программы в определенный момент работы программы.
...
* [Spring AOP](java/java-spring-AOP.md)
...
### Code Scattering
Code Scattering - разбросанность служебного функционала по всему проекту. При необходимости внесения изменений в служебный функционал изменения необходимо вносить в различных местах кода. Для решения этой проблемы используется [AOP](#aop).

### Code Tangling
Code Tangling - переплетение бизнес-логики со служебной функциональностью (логирование, проверка прав доступа и пр.). В результате чего код метод становиться громоздким и его основной функционал (бизнес-логика) становится не сразу заметной. Для решения этой проблемы используется [AOP](#aop).
