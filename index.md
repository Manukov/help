...
* [Символьная ссылка / Symbolic Link](#symbolic-link)
...
* [Утилита командной строки / Utility](#utility)
* [Файловый дескриптор / File Descriptor](#file-descriptor)
* [Функция свертки / Hash Function](#hash-function)
* [Хэш-код / Hash Code](#hash-code)
* [Хэш-сумма / Hash Code](#hash-code)
* [Шаблонизатор / Template Engine](#template-engine)
...
### CLI
CLI (Command Line Interface) - это способ взаимодействия между человеком и ПК путём отправки компьютеру команд, представляющих собой последовательность символов. Команды интерпретируются с помощью специального интерпретатора, называемого [оболочкой](#shell).

Используя интерфейс командной строки можно направлять команды:
* операционной системе - команда попадает в [интерпертатору](#shell)
* [утилите](#utility) - команда направляется файлу на HD и передает туда параметры

Синонимы: "терминал", "console"/"консоль", CLI + [интерпертатор](#shell)
...
### Hash Code
Hash Code (хэш-код, хэш-сумма) - это результат обработки данных [хэш-функцией](#hash-function). Хэш-сумма используется для:
* проверки целостности данных - см. [подпись сообщения](#message-digest)
* в качестве [идентификатора](#identifier)
* для поиска
* для замены данных. Хэш-сумма заменяет собой реальные данные, например при хранении паролей
...
### MD5
MD5 (Message Digest algoritm) — алгоритм хеширования/алгоритм хэширования, который шифрует любые данные в формате 128-bit hash (контрольную сумму), которую достаточно сложно подделать. Алгоритм используется для проверки подлинности данных, когда происходит их передача в зашифрованном виде.

### Message Digest
Message Digest (дайджест сообщения) - это [хэш-сумма](#hash-code) описавающая один файл, если измениться этот файл, то измениться и [хэш-сумма](#hash-code). 

### Metadata
Metadata (метаданные) - это дополнительная информация об программном элементе (классе, интерфейсе, поле класса ...). Метаданные могут храниться:
* в коде - при помощи [аннотаций](java/Annotation.md#annotation)
* вне кода - в xml-файлах, как это делает spring
...
Хэш-код/хэш-сумма для SHA-1: 7DD987F846400079F4B03C058365A4869047B4A0

### SHA-2

Хэш-код/хэш-сумма для SHA-1: CA737F1014A48F4C0B6DD43CB177B0AFD9E5169367544C494011E3317DBF9A509CB1E5DC1E85A941BBEE3D7F2AFBC9B1
